...
# Password used for the "Я клиент" registration screen (Номер телефона /
# Пароль / Повторите пароль). The user has no pre-existing password for
# this account -- they asked us to generate one and report it back.
PASSWORD = "MasterChas_2026"

PHONE_KEYWORDS = ["телефон", "номер", "phone"]
# IMPORTANT: check REPEAT_PASSWORD_KEYWORDS before PASSWORD_KEYWORDS --
# "Повторите пароль" contains the substring "пароль" too, so checking
# PASSWORD_KEYWORDS first would misfile the repeat field as the plain one.
REPEAT_PASSWORD_KEYWORDS = ["повтор", "repeat", "confirm password"]
PASSWORD_KEYWORDS = ["пароль", "password"]
...
def discover_fields(elements, tag):
    """List every AXTextField on the current screen along with a label and
    current value, so unfamiliar screens (like a registration form we've
    never automated before) are fully logged even if our keyword-based
    auto-fill doesn't recognize every field.

    On SOME screens the field itself carries its own descriptive AXLabel
    directly (e.g. AXLabel="Номер телефона") -- that is always the most
    reliable source and must be preferred. Only when a field has no own
    AXLabel do we fall back to nearest_label_for_field's "look at the
    nearest element above it" heuristic, which is needed on screens where
    the label is a separate AXStaticText sibling instead."""
    results = []
    for el in elements:
        if not is_textfield(el):
            continue
        frame = elem_frame(el)
        own_label = el.get("AXLabel")
        if isinstance(own_label, str) and own_label.strip():
            label = own_label.strip()
            label_source = "own-AXLabel"
        else:
            label = nearest_label_for_field(elements, frame) if frame else "(no frame)"
            label_source = "nearest-element-fallback"
        value = el.get("AXValue")
        print(f"[{tag}] FIELD label={label!r} (source={label_source}) AXValue={value!r} frame={frame}", flush=True)
...
def log_interactive_elements(elements, tag):
    """Dump every non-textfield element that carries a label (buttons,
    checkboxes, static text) so an unfamiliar submit button or agreement
    checkbox whose exact wording we haven't seen is still fully visible in
    the log, even when our keyword matching finds nothing."""
    for el in elements:
        if is_textfield(el):
            continue
        role = ((el.get("role") or "") + " " + (el.get("type") or "")).strip()
        lines = elem_lines(el)
        if not lines:
            continue
        print(f"[{tag}] ELEMENT role={role!r} lines={lines} frame={elem_frame(el)}", flush=True)
...
def auto_fill_fields(elements, tag):
    ...
    for f in fields:
        ...
        if any(k in label_low for k in PHONE_KEYWORDS):
            ...
            type_text("989003100")
            time.sleep(1)
        elif any(k in label_low for k in REPEAT_PASSWORD_KEYWORDS):
            print(f"[{tag}] filling REPEAT-PASSWORD field (label={f['label']!r})", flush=True)
            tap(x, y)
            time.sleep(1)
            type_text(PASSWORD)
            time.sleep(1)
        elif any(k in label_low for k in PASSWORD_KEYWORDS):
            print(f"[{tag}] filling PASSWORD field (label={f['label']!r})", flush=True)
            tap(x, y)
            time.sleep(1)
            type_text(PASSWORD)
            time.sleep(1)
...
def main():
    ...
    log_interactive_elements(els, "reg-fields-1")
    time.sleep(1)
    screenshot("13-after-fill-1.png")
    els = describe_all("03-after-fill-1")

    step("best-effort: accept any agreement checkbox before submitting")
    tap_best_match(els, ["согласен", "соглашаюсь", "принимаю", "условия"], "agreement-checkbox-1")
    time.sleep(1)
    els = describe_all("03b-after-agreement-1")

    step("attempt to advance registration form (submit 1)")
    tap_best_match(els, ["далее", "продолж", "зарегистрир", "готово", "отправ", "создать аккаунт"], "register-submit-1")
    time.sleep(3)
    ...
    log_interactive_elements(els, "reg-fields-2")
    time.sleep(1)
    screenshot("15-after-fill-2.png")
    els = describe_all("05-after-fill-2")

    step("best-effort: accept any agreement checkbox before submitting (2)")
    tap_best_match(els, ["согласен", "соглашаюсь", "принимаю", "условия"], "agreement-checkbox-2")
    time.sleep(1)
    els = describe_all("05b-after-agreement-2")

    step("attempt to advance registration form (submit 2)")
    tap_best_match(els, ["далее", "продолж", "зарегистрир", "готово", "отправ", "подтверд", "создать аккаунт"], "register-submit-2")
    time.sleep(3)
    ...
    log_interactive_elements(els, "reg-fields-3")
    time.sleep(1)
    screenshot("17-after-fill-3.png")
    els = describe_all("07-after-fill-3")

    step("attempt to advance registration form (submit 3)")
    tap_best_match(els, ["далее", "продолж", "зарегистрир", "готово", "отправ", "подтверд", "создать аккаунт"], "register-submit-3")
    time.sleep(3)
    screenshot("18-after-submit-3.png")
    els = describe_all("08-after-submit-3")
    log_interactive_elements(els, "reg-fields-3-post-submit")

    step("wait to see whatever screen we've landed on (this is the HOME / Главная screenshot)")
    time.sleep(4)
    screenshot("20-home-glavnaya.png")
    els = describe_all("09-post-registration")

    # Run #7 showed this app has no literal "Каталог" tab -- the bottom nav
    # is Главная / Услуги / Чаты / Профиль. A generic ["каталог","catalog"]
    # keyword match previously hit an unrelated promotional text blob
    # ("Каталог инструментов для любых задач" inside a big merged card) and
    # landed on the wrong screen ("Аукцион мастеров"). "Услуги" (Services)
    # is this app's actual catalog-of-services screen, so target that tab
    # by name directly instead of the generic catalog keyword.
    step("tap Услуги (Services) tab -- closest equivalent to a Каталог screen in this app")
    tap_best_match(els, ["услуги"], "services-tab-attempt")
    time.sleep(3)
    screenshot("21-services-catalog.png")
    describe_all("10-services-tab")

    step("done")
    screenshot("30-final-state.png")
    print("automate.py finished (registration + home/services screenshots) -- inspect screenshots for App Store Connect", flush=True)
