...
def main():
    ...
    # The user confirmed the phone number 989003100 has not been registered
    # as a master yet -- the code umRJO.1711 is not a login PIN (the login
    # screen's "Код входа" field is a 4-digit-only PIN input, incompatible
    # with this 10-character alphanumeric code). We need the registration
    # flow instead. The user explicitly said: do NOT go in via "Стать
    # мастером" -- go in via "Стать пользователем" (become a regular user)
    # instead, so try that entry point specifically.
    step("tap Стать пользователем (registration entry -- NOT Стать мастером, per user)")
    if has_match(els, ["стать пользователем", "пользователем"]):
        tap_best_match(els, ["стать пользователем", "пользователем"], "register-entry")
    else:
        print("[register-entry] 'Стать пользователем' not found on this screen, "
              "logging full element list for inspection (NOT falling back to Стать мастером)", flush=True)
        for el in els:
            lines = elem_lines(el)
            if lines:
